...
if (!empty($subscriptions)) {
  ...
  <div class="mp_wrapper">
		<h3 class="al-account__title"><?php esc_html_e('Subscriptions', 'memberpress'); ?></h3>
    <table id="mepr-account-subscriptions-table" class="mepr-account-table">
      ...
      <thead>
        <tr>
          <th scope="col"><?php echo esc_html_x('Plan', 'ui', 'memberpress'); ?></th>
          <th scope="col"><?php echo esc_html_x('Status', 'ui', 'memberpress'); ?></th>
          <?php /* <th scope="col"><?php echo esc_html_x('Active', 'ui', 'memberpress'); ?></th> */ ?>
              ...
              <div class="mepr-account-auto-rebill">
                ...
                if ($txn instanceof MeprTransaction && $txn->is_sub_account()) :
                  ...
                  if ($is_sub) :
                    ...
                    $stopped_statuses = ['stopped', 'cancelled', 'canceled'];

                    if ($s->status === MeprSubscription::$active_str || $s->status === 'active') {
                      $status_label = __('Active', 'armoredlaw');
                      $status_class = 'is-active';
                    } elseif (in_array($s->status, $stopped_statuses, true)) {
                      $status_label = __('Canceled', 'armoredlaw');
                      $status_class = 'is-canceled';
                    } else {
                      $status_label = MeprAppHelper::human_readable_status($s->status, 'subscription');
                      $status_class = 'is-' . sanitize_html_class($s->status);
                    }

                    ?>
                    <span class="mepr-subscription-status <?php echo esc_attr($status_class); ?>">
                      <?php echo esc_html($status_label); ?>
                    </span>

                  <?php
                  elseif (is_null($s->expires_at) || $s->expires_at === MeprUtils::db_lifetime()) :
            ...
            <td data-label="<?php echo esc_attr_x('Actions', 'ui', 'memberpress'); ?>">
              <div class="mepr-account-actions">
                ...
                if ($txn instanceof MeprTransaction && $txn->is_sub_account()) {
                  ...
                  // ✅ RENEW for cancelled/inactive rows (works even if $is_sub is false)
                  if (
                    (!empty($s->status) && $s->status === MeprSubscription::$cancelled_str) ||
                    (isset($s->active) && strpos($s->active, 'mepr-inactive') !== false)
                  ) {
                    if (!empty($prd->ID)) {
                      $renew_url = $prd->url();

                      // fallback: sometimes $prd->url() returns product permalink instead of /register/...
                      if (strpos($renew_url, '/register/') === false) {
                        $product_post = get_post($prd->ID);
                        if ($product_post && !is_wp_error($product_post)) {
                          $renew_url = home_url('/register/' . $product_post->post_name . '/');
                        }
                      }
                      ?>
                      <a href="<?php echo esc_url($renew_url); ?>"
                         class="btn mepr-account-row-action mepr-account-renew">
                        <?php echo esc_html__('Renew', 'armoredlaw'); ?>
                      </a>
                      <?php
                    }

                  // 1) Active / Suspended subscriptions -> existing gateway actions (Cancel, etc.)
                  } elseif (
                  ...
                  // 2) Non-subscription / one-time renewal logic (your existing code)
                  } elseif (!$is_sub && !empty($prd->ID)) {

                    if ($prd->is_one_time_payment() && $prd->is_renewable() && $prd->is_renewal()) {
                      ?>
                      <a href="<?php echo esc_url($prd->url()); ?>"
                         class="mepr-account-row-action mepr-account-renew">
                        <?php echo esc_html_x('Renew', 'ui', 'memberpress'); ?>
                      </a>
                      <?php
                    }
                  ...
                  // 3) Fallback: existing purchase/re-subscribe logic (your existing code)
                  } else {
    ...
    <div id="mepr-subscriptions-paging">
      ...
      <?php echo esc_html__('You can change, renew, or cancel your membership anytime.', 'armoredlaw'); ?>
    </p>
